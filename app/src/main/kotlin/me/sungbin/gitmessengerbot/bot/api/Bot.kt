...
import me.sungbin.gitmessengerbot.bot.BotUtil
...
class Bot(private val context: Context){
    ...
    fun reply(room: String, message: String) {
        BotUtil.replyToSession(context, StackManager.sessions[room]!!, message)
    }

    fun replyShowAll(room: String, message: String, message2: String) {
        BotUtil.replyToSession(context, StackManager.sessions[room]!!, "$message$showAll$message2")
    }
}
